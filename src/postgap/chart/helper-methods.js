...
export default {
  getBaseProps(props, fallbackProps) { // eslint-disable-line max-statements
    ...
    const initialChildProps = { parent: {
      domain, scale, width, height, data, standalone, theme, polar, origin, padding
    } };

    return data.reduce((childProps, datum, index) => {
      const eventKey = datum.eventKey || index;
      const start = scale.x(datum._start);
      const end = scale.x(datum._end);
      const exons = datum._exons.map(exon => {
          return {
              start: scale.x(exon.start),
              end: scale.x(exon.end),
              id: exon.id,
          };
      })
    ...
    //   const y = scale.y(Math.max(datum._open, datum._close));
        ...
        // const xStart = scale.x(datum.start);
        // const xEnd = scale.x(datum.end);
        // const exons = datum.Exon.map(exon => {
        //     return {
        //         ...exon,
        //         xStart: scale
        //     }
    ...
    //     index, padding, width, polar, origin
    //   };
      const dataProps = {
        start, end, exons, scale, data, datum, groupComponent,
        index, padding, width, polar, origin
      };
  ...
  getData(props) {
    ...
    id: Helpers.createAccessor(d => d.id),
      start: Helpers.createAccessor(d => d.start),
      end: Helpers.createAccessor(d => d.end),
    };
    return (props.data.map((datum, index) => {
        const canonical = datum.Transcript.filter(d => (d.is_canonical === 1))[0];
        const _start = accessor.start(canonical);
        const _end = accessor.end(canonical);
        const _exons = canonical.Exon.map(exon => ({
            start: accessor.start(exon),
            end: accessor.end(exon),
            id: accessor.id(exon),
        }));
